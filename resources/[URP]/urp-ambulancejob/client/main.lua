...
AddEventHandler('playerSpawned', function()
	...
	if FirstSpawn then
		...
		URPCore.TriggerServerCallback('urp-ambulancejob:getDeathStatus', function(isDead)
...
function OnPlayerDeath()
	...
	TriggerServerEvent('urp-ambulancejob:setDeathState', true)
...
function resetrelations()
    Citizen.Wait(1000)
    if URPCore.GetPlayerData().job.name == 'police' or URPCore.GetPlayerData().job.name == 'ambulance' then
...
function releaseBody()
    ...
    URPCore.TriggerServerCallback("urp-jail:retrieveTime", function(inJail, newJailTime)
		if inJail then
...
function StartDeathTimer()
	...
	if Config.EarlyRespawnFine then
		URPCore.TriggerServerCallback('urp-ambulancejob:checkBalance', function(canPay)
			canPayFine = canPay
		end)
	end

	local earlySpawnTimer = URPCore.Math.Round(Config.EarlyRespawnTimer / 1000)
	local bleedoutTimer = URPCore.Math.Round(Config.BleedoutTimer / 1000)
	...
	Citizen.CreateThread(function()
		...
		while bleedoutTimer > 0 and IsDead do
			...
			elseif Config.EarlyRespawnFine and canPayFine then
				text = text .. _U('respawn_bleedout_fine', URPCore.Math.GroupDigits(Config.EarlyRespawnFineAmount))

				if IsControlPressed(0, Keys['H']) and timeHeld > 60 then
					TriggerServerEvent('urp-ambulancejob:payFine')
					RemoveItemsAfterRPDeath()
...
function RemoveItemsAfterRPDeath()
	TriggerServerEvent('urp-ambulancejob:setDeathState', false)
	...
	Citizen.CreateThread(function()
		...
		URPCore.TriggerServerCallback('urp-ambulancejob:removeItemsAfterRPDeath', function()
			local playerpos = GetEntityCoords( GetPlayerPed(-1) )
				
			URPCore.SetPlayerData('lastPosition', playerpos)
			URPCore.SetPlayerData('loadout', {})
			RespawnPed(PlayerPedId(), playerpos, Config.RespawnPoint.heading)

			TriggerServerEvent('urp:updateLastPosition', playerpos)
...
RegisterNetEvent('urp-ambulancejob:useItem')
AddEventHandler('urp-ambulancejob:useItem', function(itemName)
	URPCore.UI.Menu.CloseAll()
	...
	if itemName == 'medikit' then
		...
		exports["urp-taskbar"]:taskBar(10000, "Healing")
...
function RespawnPed(ped, coords, heading)
	...
	TriggerEvent('urp-hospital:client:RemoveBleed', ped) 
	TriggerEvent('urp-hospital:client:ResetLimbs', ped)

	URPCore.UI.Menu.CloseAll()
end

AddEventHandler('urp:onPlayerDeath', function(data)
	OnPlayerDeath()
end)
...
RegisterNetEvent('urp-ambulancejob:revive')
AddEventHandler('urp-ambulancejob:revive', function()
	attemptRevive()
end)
