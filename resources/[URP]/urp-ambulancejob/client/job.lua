...
function OpenAmbulanceActionsMenu()
	...
	if Config.EnablePlayerManagement and URPCore.PlayerData.job.grade_name == 'boss' then
		table.insert(elements, {label = _U('boss_actions'), value = 'boss_actions'})
	end

	URPCore.UI.Menu.CloseAll()

	URPCore.UI.Menu.Open('default', GetCurrentResourceName(), 'ambulance_actions', {
		title    = _U('ambulance'),
		align    = 'bottom-right',
		elements = elements
	}, function(data, menu)
		...
		elseif data.current.value == 'boss_actions' then
			TriggerEvent('urp-society:openBossMenu', 'ambulance', function(data, menu)
				menu.close()
			end, {wash = false})
		end
...
function OpenMobileAmbulanceActionsMenu()
	...
	URPCore.UI.Menu.Open('default', GetCurrentResourceName(), 'mobile_ambulance_actions', {
		title    = _U('ambulance'),
		align    = 'bottom-right',
		elements = {
		...
		if data.current.value == 'citizen_interaction' then
			URPCore.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer == -1 or closestDistance > 2.0 then
					...
					if data.current.value == 'revive' then
						...
						URPCore.TriggerServerCallback('urp-ambulancejob:getItemAmount', function(quantity)
							if quantity > 0 then
								...
								if IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_a", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_b", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_c", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_d", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_e", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_f", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_g", 3) or IsEntityPlayingAnim(closestPlayerPed, "dead", "dead_h", 3)then
									...
									for i=1, 15, 1 do
										...
										URPCore.Streaming.RequestAnimDict(lib, function()
									...
									TriggerServerEvent('urp-ambulancejob:removeItem', 'medikit')
						...
						URPCore.TriggerServerCallback('urp-ambulancejob:getItemAmount', function(quantity)
							if quantity > 0 then
								...
								if health > 0 then
									...
									TriggerServerEvent('urp-ambulancejob:removeItem', 'bandage')
...
function FastTravel(coords, heading)
	...
	URPCore.Game.Teleport(playerPed, coords, function()
		DoScreenFadeIn(800)
...
Citizen.CreateThread(function()
	while true do
		...
		if URPCore.PlayerData.job and URPCore.PlayerData.job.name == 'ambulance' then
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastHospital ~= currentHospital or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				...
				then
					TriggerEvent('urp-ambulancejob:hasExitedMarker', LastHospital, LastPart, LastPartNum)
					hasExited = true
				end

				HasAlreadyEnteredMarker, LastHospital, LastPart, LastPartNum = true, currentHospital, currentPart, currentPartNum

				TriggerEvent('urp-ambulancejob:hasEnteredMarker', currentHospital, currentPart, currentPartNum)
...
AddEventHandler('urp-ambulancejob:hasEnteredMarker', function(hospital, part, partNum)
	if URPCore.PlayerData.job and URPCore.PlayerData.job.name == 'ambulance' then
...
AddEventHandler('urp-ambulancejob:hasExitedMarker', function(hospital, part, partNum)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			URPCore.ShowHelpNotification(CurrentActionMsg)
		...
		elseif URPCore.PlayerData.job ~= nil and URPCore.PlayerData.job.name == 'ambulance' and not IsDead then
...
function OpenCloakroomMenu()
	URPCore.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
		title    = _U('cloakroom'),
		align    = 'bottom-right',
		elements = {
...
function OpenVehicleSpawnerMenu(hospital, partNum)
	...
	URPCore.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle', {
		...
		if data.current.action == 'buy_vehicle' then
			...
			local authorizedVehicles = Config.AuthorizedVehicles[URPCore.PlayerData.job.grade_name]

			if #authorizedVehicles > 0 then
				for k,vehicle in ipairs(authorizedVehicles) do
					table.insert(shopElements, {
						label = ('%s - <span style="color:green;">%s</span>'):format(vehicle.label, _U('shop_item', URPCore.Math.GroupDigits(vehicle.price))),
						name  = vehicle.label,
						model = vehicle.model,
						price = vehicle.price,
						type  = 'car'
		...
		elseif data.current.action == 'garage' then
			...
			URPCore.TriggerServerCallback('urp-vehicleshop:retrieveJobVehicles', function(jobVehicles)
				if #jobVehicles > 0 then
					...
					URPCore.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_garage', {
						title    = _U('garage_title'),
						align    = 'bottom-right',
						elements = garage
						...
						if data2.current.stored then
							...
							if foundSpawn then
								...
								URPCore.Game.SpawnVehicle(data2.current.model, spawnPoint.coords, spawnPoint.heading, function(vehicle)
									URPCore.Game.SetVehicleProperties(vehicle, data2.current.vehicleProps)

									TriggerServerEvent('urp-vehicleshop:setJobVehicleState', data2.current.vehicleProps.plate, false)
...
function StoreNearbyVehicle(playerCoords)
	local vehicles, vehiclePlates = URPCore.Game.GetVehiclesInArea(playerCoords, 30.0), {}
		...
		for k,v in ipairs(vehicles) do
			...
			if GetVehicleNumberOfPassengers(v) == 0 and IsVehicleSeatFree(v, -1) then
				table.insert(vehiclePlates, {
					vehicle = v,
					plate = URPCore.Math.Trim(GetVehicleNumberPlateText(v))
				})
			end
		end
	else
	...
	URPCore.TriggerServerCallback('urp-ambulancejob:storeNearbyVehicle', function(storeSuccess, foundNum)
		if storeSuccess then
			local vehicleId = vehiclePlates[foundNum]
			local attempts = 0
			URPCore.Game.DeleteVehicle(vehicleId.vehicle)
			IsBusy = true
			...
			while DoesEntityExist(vehicleId.vehicle) do
				...
				vehicles = URPCore.Game.GetVehiclesInArea(playerCoords, 30.0)
				if #vehicles > 0 then
					for k,v in ipairs(vehicles) do
						if URPCore.Math.Trim(GetVehicleNumberPlateText(v)) == vehicleId.plate then
							URPCore.Game.DeleteVehicle(v)
							break
						end
					end
				end
...
function GetAvailableVehicleSpawnPoint(hospital, part, partNum)
	...
	for i=1, #spawnPoints, 1 do
		if URPCore.Game.IsSpawnPointClear(spawnPoints[i].coords, spawnPoints[i].radius) then
...
function OpenHelicopterSpawnerMenu(hospital, partNum)
	...
	URPCore.UI.Menu.Open('default', GetCurrentResourceName(), 'helicopter_spawner', {
		...
		if data.current.action == 'buy_helicopter' then
			...
			local authorizedHelicopters = Config.AuthorizedHelicopters[URPCore.PlayerData.job.grade_name]

			if #authorizedHelicopters > 0 then
				for k,helicopter in ipairs(authorizedHelicopters) do
					table.insert(shopElements, {
						label = ('%s - <span style="color:green;">%s</span>'):format(helicopter.label, _U('shop_item', URPCore.Math.GroupDigits(helicopter.price))),
		...
		elseif data.current.action == 'garage' then
			...
			URPCore.TriggerServerCallback('urp-vehicleshop:retrieveJobVehicles', function(jobVehicles)
				if #jobVehicles > 0 then
					...
					URPCore.UI.Menu.Open('default', GetCurrentResourceName(), 'helicopter_garage', {
...
function OpenShopMenu(elements, restoreCoords, shopCoords)
	...
	URPCore.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		title    = _U('vehicleshop_title'),
		align    = 'bottom-right',
		elements = elements
	}, function(data, menu)

		URPCore.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop_confirm', {
			...
			if data2.current.value == 'yes' then
				...
				local vehicle  = GetVehiclePedIsIn(playerPed, false)
				local props    = URPCore.Game.GetVehicleProperties(vehicle)
				props.plate    = newPlate

				URPCore.TriggerServerCallback('urp-ambulancejob:buyJobVehicle', function (bought)
					if bought then
						URPCore.ShowNotification(_U('vehicleshop_bought', data.current.name, URPCore.Math.GroupDigits(data.current.price)))
						...
						URPCore.Game.Teleport(playerPed, restoreCoords)
					else
		...
		URPCore.Game.SpawnLocalVehicle(data.current.model, shopCoords, 0.0, function(vehicle)
	...
	URPCore.Game.SpawnLocalVehicle(elements[1].model, shopCoords, 0.0, function(vehicle)
		table.insert(spawnedVehicles, vehicle)
		TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
		FreezeEntityPosition(vehicle, true)
	end)
...
function WarpPedInClosestVehicle(ped)
	...
	local vehicle, distance = URPCore.Game.GetClosestVehicle(coords)
