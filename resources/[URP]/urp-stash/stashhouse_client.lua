...
RegisterCommand('test', function()
    for i = 1, #stashHouse do
local tier = stashHouse[i].tier
print(tier)
    end
end)
...
RegisterCommand('createstash', function(source, args)
    playerId = PlayerPedId()
    playerCoords = GetEntityCoords(playerId)
    TriggerServerEvent("npstash:requestStashCreate", args[1],json.encode(GetEntityCoords(PlayerPedId(), true)), args[2])
    TriggerEvent("notification", 'Please wait.', 2)
    Citizen.Wait(1000)
    TriggerEvent("notification", 'Stashouse created with '..args[2].." pincode.", 2)
end)