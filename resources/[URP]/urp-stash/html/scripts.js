...
function keypadSubmit() {
  console.log(authCodeField.val())
  if (lockedOut) {
    return;
  }
  console.log('this is valid pin ',validPins)
  let bElevated = false;
  if (parseInt(validPins) == parseInt(authCodeField.val())) {
    bElevated = true;
  }
  
  if ((parseInt(validPins) == parseInt(authCodeField.val()))||(parseInt(validPins) == parseInt(authCodeField.val()))) {
...
function pinFailure() {
  $.post('http://urp-stash/failure', JSON.stringify({pinResult: false}));
  gameOver = true;
  $('#modal').fadeIn();
  ++failedAttempts;
  if (failedAttempts >= 3)
...
function pinSuccess(pElevated) {
  $.post('http://urp-stash/complete', JSON.stringify({pinResult: true, owner: pElevated}));
...
document.onkeyup = function (data) {
  if (data.which == 27 ) {
    $.post('http://urp-stash/close', JSON.stringify({pinResult: false}));
  }
};
