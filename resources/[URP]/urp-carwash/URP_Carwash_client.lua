...
function URP_Carwash_DrawSubtitleTimed(m_text, showtime)
	SetTextEntry_2('STRING')
	AddTextComponentString(m_text)
	DrawSubtitleTimed(showtime, 1)
end

function URP_Carwash_DrawNotification(m_text)
			...
			for i = 1, #vehicleWashStation do
				...
				if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), garageCoords2[1], garageCoords2[2], garageCoords2[3], true ) < 5 then
					...
					if IsControlJustPressed(1, Key) then
						TriggerServerEvent('URP_Carwash:checkmoney')
					end
				else
...
RegisterNetEvent('URP_Carwash:success')
AddEventHandler('URP_Carwash:success', function (price)
...
RegisterNetEvent('URP_Carwash:notenoughmoney')
AddEventHandler('URP_Carwash:notenoughmoney', function (moneyleft)
