...
Citizen.CreateThread(function()
	while true do 
		Citizen.Wait(1000)
		if PlayerData.job.label == "ems" and not isdoc then
				isdoc = true
				curDoctors = curDoctors + 1
		end
	end
end)
...
function DoHospitalCheck(typeofcheck)
	...
	local finished = exports["urp-taskbar"]:taskBar(10000,"Checking Credentials")
    if finished == 100 then
		if GetEntityHealth(PlayerPedId()) < GetEntityMaxHealth(PlayerPedId()) and skipCheckup and typeofcheck == mychecktype then
			skipCheckup = false
