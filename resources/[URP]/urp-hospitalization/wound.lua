...
function CheckDamage(ped, bone, weapon)
    ...
    if Config.Bones[bone] ~= nil then
        if not BodyParts[Config.Bones[bone]].isDamaged then
            ...
            TriggerServerEvent('urp-hospital:server:SyncInjuries', {
                limbs = BodyParts,
...
RegisterNetEvent('urp-hospital:client:SyncBleed')
AddEventHandler('urp-hospital:client:SyncBleed', function(bleedStatus)
    isBleeding = tonumber(bleedStatus)
    DoBleedAlert()
end)

RegisterNetEvent('urp-hospital:client:FieldTreatLimbs')
AddEventHandler('urp-hospital:client:FieldTreatLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 1
    end
...
RegisterNetEvent('urp-hospital:client:FieldTreatBleed')
AddEventHandler('urp-hospital:client:FieldTreatBleed', function()
    if isBleeding > 1 then
...
RegisterNetEvent('urp-hospital:client:ReduceBleed')
AddEventHandler('urp-hospital:client:ReduceBleed', function()
    if isBleeding > 0 then
        isBleeding = tonumber(isBleeding) - 1
...
RegisterNetEvent('urp-hospital:client:ResetLimbs')
AddEventHandler('urp-hospital:client:ResetLimbs', function()
    for k, v in pairs(BodyParts) do
        v.isDamaged = false
        v.severity = 0
    end
...
RegisterNetEvent('urp-hospital:client:RemoveBleed')
AddEventHandler('urp-hospital:client:RemoveBleed', function()
    isBleeding = 0
...
RegisterNetEvent('urp-hospital:client:UsePainKiller')
AddEventHandler('urp-hospital:client:UsePainKiller', function(tier)
    if tier < 4 then
...
RegisterNetEvent('urp-hospital:client:UseAdrenaline')
AddEventHandler('urp-hospital:client:UseAdrenaline', function(tier)
    if tier < 4 then
        onDrugs = 180 * tier
    end
