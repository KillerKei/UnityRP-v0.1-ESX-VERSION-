...
Citizen.CreateThread(function()
    while URPCore == nil or URPCore.PlayerData == nil or URPCore.PlayerData.job == nil do
        Citizen.Wait(1)
    end
end)
...
AddEventHandler('RunUseItem', function(itemid, slot, inventoryName, isWeapon)
    ...
    if (itemid == "highgradefemaleseed" ) then
        ...
        TriggerServerEvent("urp-weedplants:highgradefemaleseed",itemid)
        end
    end
    --
    ...
    if (itemid == "cr") then
        TriggerEvent("urp-doorlock:UseRedKeycard", itemid)
    end
    ...
    if (itemid == "Gruppe6Card") then
        TriggerServerEvent('urp-securityheists:banktstart')
    end
            ...
            Citizen.CreateThread(function()
                while finished ~= 100 and not cancelNos do
                    ...
                    if GetEntitySpeed(GetVehiclePedIsIn(player, false)) > 11 then
                        exports["urp-taskbar"]:closeGuiFail()
                        cancelNos = true
                    end
                end
            end)
            finished = exports["urp-taskbar"]:taskBar(20000,"Nitrous")
    ...
    if (itemid == "bandage") then
        TaskItem("amb@world_human_clipboard@male@idle_a", "idle_c", 49,10000,"Healing","urp-hospital:items:bandage",true,itemid,playerVeh)
    end
    ...
    if (itemid == "methbag") then
        local finished = exports["urp-taskbarskill"]:taskBar(2500,10)
        if (finished == 100) then  
            TriggerEvent("attachItemObjectnoanim","crackpipe01")
            TriggerEvent("Evidence:StateSet",2,1200)
            TriggerEvent("Evidence:StateSet",6,1200)
...
AddEventHandler('inv:lockPick', function(isForced,inventoryName,slot)
    ...
    if targetVehicle == 0 then
        ...
        if driverPed ~= 0 then
            ...
            local finished = exports["urp-taskbarskill"]:taskBar(25000,3)
            ...
            local finished = exports["urp-taskbarskill"]:taskBar(2200,10)
        ...
        if targetVehicle ~= 0 and not isForced then
            ...
            local finished = exports["urp-taskbarskill"]:taskBar(math.random(5000,25000),math.random(10,20))
            if finished ~= 100 then
                 lockpicking = false
                return
            end
            ...
            TriggerEvent("civilian:alertPolice",20.0,"lockpick",targetVehicle)
            local finished = exports["urp-taskbarskill"]:taskBar(1500,math.random(5,15))
            if finished ~= 100 then
...
AddEventHandler('veh:repairing', function(inventoryName,slot,itemid)
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.5 then
            ...
            local finished = exports["urp-taskbarskill"]:taskBar(15000,math.random(10,20))
            if finished ~= 100 then
                fixingvehicle = false
                reapiring = false
            ...
            if finished == 100 then
                ...
                if myJob == "towtruck" then
                    ...
                    if advanced then
                        ...
                        local finished = exports["urp-taskbarskill"]:taskBar(timer,math.random(5,15))
                        if finished ~= 100 then
                            fixingvehicle = false
                            reapiring = false
                            ClearPedTasks(playerped)
...
RegisterCommand("seizecash", function(source, args)
    local player = URPCore.GetPlayerData()
    local closestPlayer, closestDistance = URPCore.Game.GetClosestPlayer()
    if closestPlayer ~= -1 and closestDistance <= 2.0 and URPCore.PlayerData.job.name == 'police' then
        local searchPlayerPed = GetPlayerPed(closestPlayer)
...
RegisterCommand("steal", function(source, args)
    ...
    if closestPlayer ~= -1 and closestDistance <= 1.0 then
        ...
        if IsEntityPlayingAnim(searchPlayerPed, 'missfbi5ig_22', 'hands_up_anxious_scientist', 3) or (IsEntityPlayingAnim(searchPlayerPed, "dead", "dead_a", 3)) then
            ...
            local finished = exports["urp-taskbar"]:taskBar(3500, "Robbing")
            TriggerServerEvent("people-search", GetPlayerServerId(closestPlayer)) --COPY THIS
...
RegisterNetEvent('urp-inventory:goldtrade')
AddEventHandler('urp-inventory:goldtrade', function()
		...
		if GoldBars < 1.5 then
			if isTime() then
				...
				if IsControlJustReleased(0,38) then
					if exports["urp-inventory"]:hasEnoughOfItem("goldbar",70,true) then
						local finished = exports["urp-taskbar"]:taskBar(6000,"Trading Bars")
						if finished == 100 then
                            TriggerEvent('urp-inventory:goldtrade')
						end
					end
					Citizen.Wait(1000)
				end
