...
function invSwap(targetSlot, targetInventory, originSlot, originInventory) {
  ...
  $.post("http://urp-inventory/swap", JSON.stringify(arr));
};

function removeCraftItems(itemid, moveAmount) {
  let arr = itemList[itemid].craft
  let amount = moveAmount;
  $.post("http://urp-inventory/removeCraftItems", JSON.stringify([arr, amount]));
};
...
function BuildDrop(brokenSlots) {
  $.post("http://urp-inventory/dropIncorrectItems", JSON.stringify({
    slots: brokenSlots
  }));
}
...
function UpdateSetWeights() {
  ...
  $.post("http://urp-inventory/Weight", JSON.stringify({
    weight: meh
  }));
}
...
function SwapStacks(targetSlot, originSlot, inv1, inv2) {
  // $.post('http://urp-inventory/swapstack', JSON.stringify({
  //   slot1: slot1,
  //   slot2: slot2,
  //   inv1: inv1,
  //   inv2: inv2,
...
function closeInv(pIsItemUsed = false) {
  ...
  $.post("http://urp-inventory/ServerCloseInventory", JSON.stringify({
    name: TargetInventoryName
  }));
  TargetInventoryName = "none";
  $.post("http://urp-inventory/Close", JSON.stringify({
