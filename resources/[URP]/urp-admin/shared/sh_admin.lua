...
function URP.Admin.SetStatus(self, status, src)
    ...
    if IsDuplicityVersion() then
        ...
        for k,v in pairs(URP._Admin.CurAdmins) do
            TriggerClientEvent("urp-admin:setStatus", k, src, status)
        end

        return
    end

    TriggerServerEvent("urp-admin:setStatus", status)
end

AddEventHandler("np-base:exportsReady", function()
    exports["np-base"]:addModule("Admin", URP.Admin)
    exports["np-base"]:addModule("_Admin", URP._Admin)
end)