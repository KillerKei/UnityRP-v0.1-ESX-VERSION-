      ...
      if (waitCheck > 40.0 and waitCheck2 > 40.0 ) or inprocess then
        ...
        for i = 1, #workoutAreas do
          if IsControlJustPressed(0, 38) then
            TriggerEvent('doworkout', workoutAreas[i]["x"], workoutAreas[i]["y"], workoutAreas[i]["z"], workoutAreas[i]["h"], workoutAreas[i]["workType"])
          end
...
AddEventHandler('doworkout', function(x, y, z, h, data)
    inprocess = true
    SetEntityCoords(PlayerPedId(),x,y,z)
    SetEntityHeading(PlayerPedId(),h)
    TriggerEvent("animation:PlayAnimation",data)
    Citizen.Wait(30000)
