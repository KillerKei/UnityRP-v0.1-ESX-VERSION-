...
RegisterServerEvent('urp-skin:save')
AddEventHandler('urp-skin:save', function()
    local src = source
    TriggerClientEvent('urp-clothing:guardarSkin',src)
end)

URPCore.RegisterServerCallback('urp-skin:getPlayerSkin', function(source, cb)
...
URPCore.RegisterServerCallback('urp-clothing:getSex', function(source, cb)
    local xPlayer = URPCore.GetPlayerFromId(source)
...
RegisterServerEvent("urp-clothing:insert_character_current")
AddEventHandler("urp-clothing:insert_character_current",function(data)
    if not data then return end
    local src = source
    local user = URPCore.GetPlayerFromId(src)
...
RegisterServerEvent("urp-clothing:insert_character_face")
AddEventHandler("urp-clothing:insert_character_face",function(data)
    if not data then return end
    local src = source
...
RegisterServerEvent("urp-clothing:get_character_face")
AddEventHandler("urp-clothing:get_character_face",function(pSrc)
    ...
    MySQL.Async.fetchAll("SELECT cc.model, cf.hairColor, cf.headBlend, cf.headOverlay, cf.headStructure FROM users cf INNER JOIN users cc on cc.identifier = cf.identifier WHERE cf.identifier = @identifier", {['identifier'] = characterId}, function(result)
        if (result ~= nil and result[1] ~= nil) then
            ...
            if model == 1885233650 or model == -1667301416 then
                TriggerClientEvent("urp-clothing:setpedfeatures", src, temp_data)
            end
        else
            TriggerClientEvent("urp-clothing:setpedfeatures", src, false)
        end
	end)
end)

RegisterServerEvent("urp-clothing:get_character_current")
AddEventHandler("urp-clothing:get_character_current",function(pSrc)
    local src = (not pSrc and source or pSrc)
    local user = URPCore.GetPlayerFromId(src)
    local characterId = user.identifier
    ...
    MySQL.Async.fetchAll("SELECT * FROM users WHERE identifier = @identifier", {['identifier'] = characterId}, function(result)
        ...
        TriggerClientEvent("urp-clothing:setclothes", src, temp_data,0)
	end)
end)

RegisterServerEvent("urp-clothing:retrieve_tats")
AddEventHandler("urp-clothing:retrieve_tats", function(pSrc)
    local src = (not pSrc and source or pSrc)
	local user = URPCore.GetPlayerFromId(src)
	MySQL.Async.fetchAll("SELECT * FROM playerstattoos WHERE identifier = @identifier", {['identifier'] = user.identifier}, function(result)
        if(#result == 1) then
			TriggerClientEvent("urp-clothing:settattoos", src, json.decode(result[1].tattoos))
		else
			local tattooValue = "{}"
			...
			TriggerClientEvent("urp-clothing:settattoos", src, {})
		end
	end)
end)

RegisterServerEvent("urp-clothing:set_tats")
AddEventHandler("urp-clothing:set_tats", function(tattoosList)
	local src = source
	local user = URPCore.GetPlayerFromId(src)
...
RegisterServerEvent("urp-clothing:get_outfit")
AddEventHandler("urp-clothing:get_outfit",function(slot)
    ...
    MySQL.Async.fetchAll("SELECT * FROM character_outfits WHERE steamid = @steamid and slot = @slot", {
        ...
        if result and result[1] then
            ...
            TriggerClientEvent("urp-clothing:setclothes", src, data,0)
...
RegisterServerEvent("urp-clothing:set_outfit")
AddEventHandler("urp-clothing:set_outfit",function(slot, name, data)
    if not slot then return end
    local src = source
...
RegisterServerEvent("urp-clothing:remove_outfit")
AddEventHandler("urp-clothing:remove_outfit",function(slot)
...
RegisterServerEvent("urp-clothing:list_outfits")
AddEventHandler("urp-clothing:list_outfits",function()
    local src = source
    local user = URPCore.GetPlayerFromId(src)
    local steamid = user.identifier
    local slot = slot
    ...
    MySQL.Async.fetchAll("SELECT slot, name FROM character_outfits WHERE steamid = @steamid", {['steamid'] = steamid}, function(skincheck)
    	TriggerClientEvent("urp-clothing:listOutfits",src, skincheck)
	end)
end)
...
AddEventHandler("clothing:checkIfNew", function()
    ...
    MySQL.Async.fetchAll("SELECT * FROM users WHERE identifier = @identifier LIMIT 1", {
        ...
        if user.job.name == "police" or user.job.name == "ems" then isService = true end
        ...
        if result[1] == nil then
            MySQL.Async.fetchAll("SELECT * FROM users WHERE identifier = @identifier", {["identifier"] = steamid}, function(result)
                if result[1].skin then
                    TriggerClientEvent('urp-clothing:setclothes',src,{},json.decode(result[1].skin))
                else
                    TriggerClientEvent('urp-clothing:setclothes',src,{},nil)
                end
                return
            end)
        else
            TriggerEvent("urp-clothing:get_character_current", src)
        end
        TriggerClientEvent("urp-clothing:inService",src,isService)
    end)
end)
