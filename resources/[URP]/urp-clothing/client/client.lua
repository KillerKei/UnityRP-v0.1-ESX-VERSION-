...
AddEventHandler('urp-skin:setLastSkin', function(skin)
    LastSkin = skin
  end)
...
function Save(save)
    if save then
        data = GetCurrentPed()
        TriggerServerEvent("urp-clothing:insert_character_current", data)
        if data.model == `mp_f_freemode_01` or data.model == `mp_m_freemode_01` then
            TriggerServerEvent("urp-clothing:insert_character_face", data)
            TriggerServerEvent("urp-clothing:set_tats", currentTats)
        end
    else
        LoadPed(oldPed)
    end
...
RegisterNetEvent("urp-clothing:guardarSkin")
AddEventHandler("urp-clothing:guardarSkin", function()
    data = GetCurrentPed()
    TriggerServerEvent("urp-clothing:insert_character_current", data)
    if data.model == `mp_f_freemode_01` or data.model == `mp_m_freemode_01` then
        TriggerServerEvent("urp-clothing:insert_character_face", data)
...
RegisterNetEvent("urp-clothing:inService")
AddEventHandler("urp-clothing:inService", function(service)
    isService = service
end)

RegisterNetEvent('urp-identity:identityCheck')
AddEventHandler('urp-identity:identityCheck', function(identityCheck)
	hasIdentity = identityCheck
end)

RegisterNetEvent("urp-clothing:hasEnough")
AddEventHandler("urp-clothing:hasEnough", function(menu)
...
RegisterNetEvent("urp-clothing:setclothes")
AddEventHandler("urp-clothing:setclothes", function(data,alreadyExist)
    player = GetPlayerPed(-1)
    local function setDefault()
        Citizen.CreateThread(function()
            firstChar = true
            local gender = nil
            URPCore.TriggerServerCallback('urp-clothing:getSex', function(sex)
				gender = sex
			end)
            Citizen.Wait(1000)
            if gender ~= "m" then
...
RegisterNetEvent("urp-clothing:defaultReset")
AddEventHandler("urp-clothing:defaultReset", function()
    local gender = nil
    URPCore.TriggerServerCallback('urp-clothing:getSex', function(sex)
        gender = sex
    end)
    ...
    TriggerEvent("customNotification","Looks like you picked a character, nice job! You have a Hotel booked in the city, press P to call a Taxi, Or, if you see one, hit F to jump inside it(this works anywhere). Once in the Taxi, mark the location on the map where you want to go for free! (Hotel 1).")
    SetNewWaypoint(312.96966552734,-218.2705078125)
    local dstHt = #(vector3(GetEntityCoords(PlayerPedId())) - vector3(312.96966552734,-218.2705078125,54.221797943115))
...
RegisterNetEvent("urp-clothing:settattoos")
AddEventHandler("urp-clothing:settattoos", function(playerTattoosList)
    currentTats = playerTattoosList
    SetTats(GetTats())
end)

RegisterNetEvent("urp-clothing:setpedfeatures")
AddEventHandler("urp-clothing:setpedfeatures", function(data)
    player = GetPlayerPed(-1)
    if data == false then
...
RegisterCommand("outfitadd", function(source, args, rawCommand)
    if (IsNearShop(clothingShops) < 9.0) then
        if args[1] and args[2] then
            TriggerEvent('urp-clothing:outfits', 1, args[1], args[2])
        else
...
RegisterCommand("outfituse", function(source, args, rawCommand)
    if (IsNearShop(clothingShops) < 9.0) then
        if args[1] then
            TriggerEvent('urp-clothing:outfits', 3, args[1])
        else
...
RegisterCommand("outfitdel", function(source, args, rawCommand)
    if (IsNearShop(clothingShops) < 9.0) then
        if args[1] then
            TriggerEvent('urp-clothing:outfits', 2, args[1])
        else
            TriggerEvent('DoLongHudText', "You need to do something like /outfituse 1 | 1 being the slot id that you will have had previously saved", 1)  
        end
    else
...
RegisterCommand("outfits", function(source, args, rawCommand)
    if (IsNearShop(clothingShops) < 9.0) then
        TriggerEvent('urp-clothing:outfits', 4)
    else
        TriggerEvent('DoLongHudText', "You are not near a wardrobe", 2)
    end
end, false)  

RegisterNetEvent('urp-clothing:listOutfits')
AddEventHandler('urp-clothing:listOutfits', function(skincheck)
...
RegisterNetEvent('urp-clothing:outfits')
AddEventHandler('urp-clothing:outfits', function(pAction, pId, pName)
    if pAction == 1 then
        TriggerServerEvent("urp-clothing:set_outfit",pId, pName, GetCurrentPed())
    elseif pAction == 2 then
        TriggerServerEvent("urp-clothing:remove_outfit",pId)
    elseif pAction == 3 then
        TriggerEvent('InteractSound_CL:PlayOnOne','Clothes1', 0.6)
        TriggerServerEvent("urp-clothing:get_outfit", pId)
    else
