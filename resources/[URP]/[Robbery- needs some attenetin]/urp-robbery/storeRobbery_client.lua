...
AddEventHandler("urp-jobmanager:playerBecameJob", function(job, name, notify)
...
Citizen.CreateThread(function()
	...
	local texttodraw = "Press ~g~E~w~ to decrypt"
    local distance = GetDistanceBetweenCoords(pos.x,pos.y,pos.z,x, y, z,false)
        if distance <= 1.5 then
			...
			if IsControlJustReleased(0, 86) then
				...
				if exports["urp-inventory"]:hasEnoughOfItem("pix1",1,false) then			
					TriggerEvent("Animation:typeforrob")
					local finished = exports["urp-taskbar"]:taskBar(40000,"Decrypting Data")
...
AddEventHandler("police:notifySecurityCam", function(currentRobbery)
	...
	if jobname == "police" then
		...
		TriggerEvent('chat:addMessage', {
			template = '<div style="padding: 0.475vw; padding-left: 0.8vw; padding-right: 0.7vw; margin: 0.1vw; background-color: #870c0c; border-radius: 10px 10px 10px 10px;"><span style="width: 100%; font-weight: bold;">DISPATCH: </span>Security Camera number {0} has been triggered.</div>',
			args = {currentRobbery}
		})
	end
end)
