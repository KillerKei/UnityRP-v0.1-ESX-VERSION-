...
-- dont let turkish sydres code shit he cant code dumb fuck
...
AddEventHandler("robbery:sendMarkers", function(markerSent,particlePosSent,markerDifficultSent)
    markers = vector3(markerSent.x, markerSent.y,  markerSent.z)
    particlePos = vector3(particlePosSent.x, particlePosSent.y,  particlePosSent.z)
    markerDifficult = vector3(markerDifficultSent)
    print('client markers', json.encode(markers))
    print('client particle', particlePos)
    print('client marker ', markerDifficult)
end)
...
local activePolice = 5
    ...
    if itemID == "lockpick" then
        ...
        local finished = exports["urp-lockpicking"]:lockpick(pick[1],pick[2],pick[3],pick[4])
    ...
    if itemID == "thermite" then
            ...
            local outcome = exports["urp-thermite"]:startGame(thermite[1],thermite[2],thermite[3],thermite[4])
            local finished = 0
            ...
            if finished == 100 then
                ...
                if math.random(1,100) > 85 then
                    exports["urp-thermite"]:startFireAtLocation(marker.pos[1],marker.pos[2],marker.pos[3]-0.3,10000)                   
                end
                TriggerEvent('inventory:removeItem',"thermite", 1)
            end
    ...
    if itemID == "Gruppe6Card2" then
        ...
        if exports["urp-inventory"]:hasEnoughOfItem("electronickit",1,false) then
            local card = exports["urp-taskbar"]:taskBar(9000,"Inserting Card")
            if card == 100 then
                Wait(400)
                TriggerEvent("animation:cancel")
                TriggerServerEvent("robbery:alarmTrigger",locationID)
                local rdif = exports["urp-taskbar"]:taskBar(6000,"Hacking..")
                if  rdif == 100 then
                    local kit = difficult.kit
...
function thermiteHandle(locationID,itemID)
    ...
    local taskFinished = exports["urp-taskbar"]:taskBar(1000,"Getting Thermite ready")
