...
AddEventHandler("robbery:robberyFinished", function(locationID, ToolType, itemid)
    ...
    if marker.attachedDoor ~= nil and marker.attachedDoor ~= 0 and marker.attachedDoor ~= -22 then TriggerEvent('urp-doors:ForceLockState', marker.attachedDoor, src) end
    ...
    for k,v in pairs(markers) do
        if v.group == GroupType and v.ToolType == ELECTRICAL_LOCK then
            ...
            if v.attachedDoor ~= nil and v.attachedDoor ~= -22 and v.attachedDoor ~= 0 then
                TriggerClientEvent("urp-doors:ForceLockState",v.attachedDoor,0)
            end
        end
    end
end
...
function lockdown()
    ...
    TriggerEvent("urp-doors:ForceLockState",211,1)
    TriggerEvent("urp-doors:ForceLockState",212,1)
    TriggerEvent("urp-doors:ForceLockState",213,1)
    TriggerEvent("urp-doors:ForceLockState",214,1)
end

function endLockdown()
    TriggerClientEvent("jail:lockdown", -1, false)
    TriggerEvent("urp-doors:ForceLockState",211,0)
    TriggerEvent("urp-doors:ForceLockState",212,0)
    TriggerEvent("urp-doors:ForceLockState",213,0)
    TriggerEvent("urp-doors:ForceLockState",214,0)
end

function CheckLargeBanks()
    for k,v in pairs(markers) do
        if v.attachedDoor ~= nil and v.attachedDoor ~= -22 and v.attachedDoor ~= 0 then
            if not flags[k].isFinished and not exports["urp-doors"]:isDoorLocked(v.attachedDoor) then
                TriggerEvent("urp-doors:ForceLockState", v.attachedDoor,1)
            end
        end
