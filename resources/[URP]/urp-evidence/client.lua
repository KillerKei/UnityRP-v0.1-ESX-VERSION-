    ...
    while true do
      ...
      if GetHashKey("WEAPON_FLASHLIGHT") == GetSelectedPedWeapon(ped) or triggered then
        ...
        if IsControlJustReleased(0,38) and minScan < 2.0 then
          local myjob = URP.GetPlayerData().job.name
          local finished = exports["urp-taskbar"]:taskBar(3000,"Picking Up Item","What?",true)
...
-- RegisterCommand('yeet', function(soure, args)
-- TriggerEvent('giveid', source)
-- end)
...
-- 		sex = args[4],
--   }
--   TriggerEvent("player:receiveItem","idcard",1,true,idinfo)
--   print(json.encode(idinfo))
-- end)
...
AddEventHandler("Evidence:StateSet",function(stateId,stateLength)
  if Currentstates[stateId]["timer"] < 10 and stateLength ~= 0 then
    TriggerServerEvent('statelol', Currentstates[stateId]["text"])
