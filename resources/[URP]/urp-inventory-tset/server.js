//we will just generate a table for our shop items.
//this works off the itemlist in the inventory

//the "2" declares the "2" below in inventory-open section.

//to open convenienceStore inventory we use
//mp.events.callRemote("server-inventory-open", player, "2", "shop");

//if we just want to open a player inventory, we say
// mp.events.callRemote("server-inventory-open", player, "0", "anything");
...
// onNet('onResourceStart', function(resource)
...
function functionRemoveAny(player, itemidsent, amount, openedInv,pId) {
    let src = pId
    let playerinvname = 'ply-' + player
    //////print('function REMOVE', source, player)
                ...
                let string = `SELECT firstname,lastname,sex,dateofbirth FROM characters WHERE id = '${player}'`;
                 ...
                 case "casing":
                    ...
                    case "evidence":
                        ...
                        clearTimeout(timeout)
                        return resolve(returnInfo);
					case "stolen10ctchain":
                        returnInfo = JSON.stringify({ Serial:itemdata.id, Stolen: itemdata.Stolen, other: itemdata.other })
                        timeout = 1;
                        clearTimeout(timeout)
                        return resolve(returnInfo);
					case "stolencasiowatch":
                        returnInfo = JSON.stringify({ Serial:itemdata.id, Stolen: itemdata.Stolen, other: itemdata.other })
                        timeout = 1;
                        clearTimeout(timeout)
                        return resolve(returnInfo);
...
onNet("server-inventory-give", async (player, itemid, slot, amount, generateInformation, itemdata, openedInv) => {
    console.log(player, itemid, slot, amount, generateInformation, itemdata, openedInv)
   
        print(generateInformation)
        let src = source
        let playerinvname = 'ply-' + player
        let information = "{}"
...
onNet("sendingItemstoClient", async (player, sauce) => {
    ...
    if (player == ""){}else{
        ...
        exports.ghmattimysql.execute(string,{}, function(inventory) {
            ...
            //////console.log('loging to send in current items',invArray)
        //    emitNet('inventory-update-player', src, invArray)
            // emitNet("inventory-open-target", src, [invArray, arrayCount,playerinvname,0,0,0,500,true]);
        //   emitNet("server-inventory-open", playerinvname, "0", playerinvname);
            // emitNet("inventory-open-target1", src, [invArray,arrayCount,playerinvname,invArrayTarget,0,NewDroppedName,500,false]);
    ...
    exports.ghmattimysql.execute(string,{}, function(inventory) {
        ...
        var arrayCount = 0;
           ...
           if(secondInventory == "1") {
            //////console.log('opening here 2')
               var targetinvname = targetName
               ////console.log('here we gow 1')
          ...
          //     ////console.log('here we gow 2')
              ...
              // ////console.log(JSON.stringify(invArrayTarget))
...
// checked below
// call this to add an item to a users inventory for next time they open it
...
onNet("inv:delete", async (player, cid) => {
...
onNet("server-inventory-move", async (player, data, coords) => {
    ...
    let creationDate = Date.now()
   // ////print('icecream   ',crafting,purchase)
    ...
    if (purchase) {
        if(isWeapon) {
          //  ////console.log('purchase ',purchase, isWeapon)

        }
        info = await GenerateInformation(player,itemidsent)
        removecash(source,itemCosts)
      //  ////console.log('remove cash here', source,itemCosts)
       // emit("inventory:takeMyCash", source,itemCosts)
        if (!PlayerStore) {
            for (let i = 0; i < parseInt(amount); i++) {
               // ////console.log("we move amount 1", db, itemidsent, amount) 
           ...
           // ////console.log(crafting)
            ...
            ////console.log("drops .. .. .. hidden?")    
            db(`INSERT INTO user_inventory2 SET slot='${targetslot}', name='${targetname}', dropped='1' WHERE slot='${startslot}' AND name='${startname}'`);
            emit('sendingItemstoClient', player,src)
           } else {
            db(`UPDATE user_inventory2 SET slot='${targetslot}', name='${targetname}', dropped='0' WHERE slot='${startslot}' and name='${startname}'`);
          //  ////console.log("we move amount here now ", db, itemidsent, amount) 
            ////console.log('items?',db)    
           }
        }
     } else {
        ////console.log('THIS IS CRAFTING 2', targetname, startname)
        if (crafting == true){
                     ////console.log('show craft here')
                     info - await GenerateInformation(player,itemidsent)
                    for (let i = 0; i < parseInt(amount); i++) {
                        db(`INSERT INTO user_inventory2 (item_id, name, information, slot, creationDate) VALUES ('${itemidsent}','${targetname}','${info}','${targetslot}','${creationDate}' );`);
                    }
            ...
            db(`UPDATE user_inventory2 SET slot='${targetslot}', name='${targetname}', dropped='0' WHERE slot='${startslot}' and name='${startname}'`);
          //  ////console.log("we move amount moving here in inventory", db, itemidsent, amount, targetslot, targetname, startslot, startname) 
    }
});
...
//          exports.ghmattimysql.execute(string,{}, function(result) {
//              ////console.log("RESULT ITEM ID : ",result[0].item_id)
...
function PoliceArmory() {
    var shopItems = [
		...
		{ item_id: "-1075685676", id: 0, name: "Shop", information: "{}", slot: 4, amount: 1 },
		...
		{ item_id: "armor", id: 0, name: "Shop", information: "{}", slot: 8, amount: 5 },
        { item_id: "IFAK", id: 0, name: "Shop", information: "{}", slot: 9, amount: 5 },
		{ item_id: "watch", id: 0, name: "Shop", information: "{}", slot: 10, amount: 1 },
        { item_id: "advrepairkit", id: 0, name: "Shop", information: "{}", slot: 11, amount: 5 },
		{ item_id: "doorblaster", id: 0, name: "Shop", information: "{}", slot: 12, amount: 1 },
        { item_id: "radio", id: 0, name: "Shop", information: "{}", slot: 13, amount: 1 },
		{ item_id: "sandwich", id: 0, name: "Shop", information: "{}", slot: 14, amount: 50 },
        { item_id: "hamburger", id: 0, name: "Shop", information: "{}", slot: 15, amount: 50 },
        { item_id: "cola", id: 0, name: "Shop", information: "{}", slot: 16, amount: 50 },
        { item_id: "water", id: 0, name: "Shop", information: "{}", slot: 17, amount: 50 },
        { item_id: "beer", id: 0, name: "Shop", information: "{}", slot: 18, amount: 50 },
		{ item_id: "chips", id: 0, name: "Shop", information: "{}", slot: 19, amount: 50 },
		{ item_id: "burrito", id: 0, name: "Shop", information: "{}", slot: 20, amount: 50 },
		{ item_id: "muffin", id: 0, name: "Shop", information: "{}", slot: 21, amount: 50 },
		{ item_id: "slushy", id: 0, name: "Shop", information: "{}", slot: 22, amount: 50 },
		{ item_id: "cookie", id: 0, name: "Shop", information: "{}", slot: 23, amount: 50 },
		{ item_id: "mobilephone", id: 0, name: "Shop", information: "{}", slot: 24, amount: 1 },
    ];
    return JSON.stringify(shopItems);
}
...
function Recycle() {
    var shopItems = [
        ...
        { item_id: "rubber", id: 0, name: "craft", information: "{}", slot: 3, amount: 15 },
        { item_id: "electronics", id: 0, name: "craft", information: "{}", slot: 4, amount: 15 },
        { item_id: "steel", id: 0, name: "craft", information: "{}", slot: 5, amount: 15 },
		{ item_id: "copper", id: 0, name: "craft", information: "{}", slot: 6, amount: 15 },
		{ item_id: "glass", id: 0, name: "craft", information: "{}", slot: 7, amount: 15 },
    ];
    return JSON.stringify(shopItems);
}
...
function CraftBikerStoreGangs() {
    var shopItems = [
        { item_id: "advlockpick", id: 0, name: "craft", information: "{}", slot: 1, amount: 1 },
        { item_id: "lockpick", id: 0, name: "craft", information: "{}", slot: 2, amount: 1 },
        { item_id: "drill", id: 0, name: "craft", information: "{}", slot: 3, amount: 5 },
        { item_id: 2343591895, id: 0, name: "craft", information: "{}", slot: 4, amount: 1 },
        { item_id: 2508868239, id: 0, name: "craft", information: "{}", slot: 5, amount: 1 },
    ];
    return JSON.stringify(shopItems);
};
