...
function HasStolenGoods()
	if OxyRun then
		...
		for i = 84, 97 do
			local itemcount = exports["urp-inventory"]:hasEnoughOfItem(stolenGoodsTable[i],1,false)
			if itemcount then
	...
	if DoesEntityExist(deliveryPed) and not IsEntityDead(deliveryPed) then
		...
		if HasStolenGoods() then
			...
			if OxyRun then
				...
				if exports["urp-inventory"]:hasEnoughOfItem("inkedmoneybag",1,false) then     
					-- RepPlus()
					TriggerEvent("inventory:removeItem","inkedmoneybag", 1)   
					cashPayment = cashPayment + 85000  

		        elseif exports["urp-inventory"]:hasEnoughOfItem("rollcash",30,false) then     
		        ...
		        elseif exports["urp-inventory"]:hasEnoughOfItem("markedbills",20,false) then     
		            TriggerEvent("inventory:removeItem","markedbills", 20)   
		            cashPayment = cashPayment + markedbillsprice            
		            TriggerEvent("DoLongHudText","Thanks for the extra sauce!")
				elseif exports["urp-inventory"]:hasEnoughOfItem("band",20,false) then     
    ...
    while timer > 0 do
        ...
        if not searching then
            ...
            if IsControlJustReleased(1,38) and #(pos - GetEntityCoords(PlayerPedId())) < 3.0 then
                ...
                local finished = exports["urp-taskbar"]:taskBar(15000,"Searching Thug")
...
function CompleteScrapping(vehicle, originalVehicleLocation)
	...
	local finished = exports["urp-taskbar"]:taskBar(20000,"Scrapping Car")
	  ...
	  --  local dropOff3 = #(vector3(GetEntityCoords(PlayerPedId())) - vector3(chopinfo[3]["x"],chopinfo[3]["y"],chopinfo[3]["z"]))
		...
		if GoldBars < 1.5 then
			...
			if GoldBarTime then
				...
				if IsControlJustReleased(0,38) then
					if exports["urp-inventory"]:hasEnoughOfItem("goldbar",70,true) then
						local finished = exports["urp-taskbar"]:taskBar(120000,"Trading Bars")
						if finished == 100 then
		...
		if dropOff4 < 1.5 then
			DrawText3Ds(590.71, 2739.6, 15.86, "[E] to Leave") 
			if IsControlJustReleased(0,38) then
				CleanUpArea()
