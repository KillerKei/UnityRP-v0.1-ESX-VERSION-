...
RegisterServerEvent('urp-identity:setIdentity')
AddEventHandler('urp-identity:setIdentity', function(data, myIdentifiers)
	setIdentity(myIdentifiers.steamid, data, function(callback)
		if callback then
			TriggerClientEvent('urp-identity:identityCheck', myIdentifiers.playerid, true)
		else
...
AddEventHandler('es:playerLoaded', function(source)
	...
	TriggerClientEvent('urp-identity:saveID', source, myID)
	getIdentity(source, function(data)
		if data.firstname == '' then
			TriggerClientEvent('urp-identity:identityCheck', source, false)
			TriggerClientEvent('urp-identity:showRegisterIdentity', source)
		else
			TriggerClientEvent('urp-identity:identityCheck', source, true)
		end
	end)
end)
...
AddEventHandler('onResourceStart', function(resource)
	if resource == GetCurrentResourceName() then
		...
		local xPlayers, xPlayer = URPCore.GetPlayers()
		for i=1, #xPlayers, 1 do
			xPlayer = URPCore.GetPlayerFromId(xPlayers[i])
			...
			TriggerClientEvent('urp-identity:saveID', xPlayer.source, myID)

			getIdentity(xPlayer.source, function(data)
				if data.firstname == '' then
					TriggerClientEvent('urp-identity:identityCheck', xPlayer.source, false)
					TriggerClientEvent('urp-identity:showRegisterIdentity', xPlayer.source)
				else
					TriggerClientEvent('urp-identity:identityCheck', xPlayer.source, true)
				end
			end)
		end
	end
...
RegisterServerEvent("urp-identity:UpdateCid")
AddEventHandler('urp-identity:UpdateCid', function()
    local src = source
    local cid = getCid(GetPlayerIdentifiers(src)[1])
    TriggerClientEvent('updatecid', src, cid)
end)

RegisterServerEvent("urp-identity:UpdateJob")
AddEventHandler('urp-identity:UpdateJob', function()
    local src = source
	local job = getJob(GetPlayerIdentifiers(src)[1])
    TriggerClientEvent('urp:playerBecameJob', src, job)
end)
