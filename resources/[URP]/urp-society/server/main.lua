...
AddEventHandler('urp-society:registerSociety', function(name, label, account, datastore, inventory, data)
	local found = false
...
AddEventHandler('urp-society:getSocieties', function(cb)
	cb(RegisteredSocieties)
end)

AddEventHandler('urp-society:getSociety', function(name, cb)
	cb(GetSociety(name))
end)

RegisterServerEvent('urp-society:withdrawMoney')
AddEventHandler('urp-society:withdrawMoney', function(society, amount)
	local xPlayer = URPCore.GetPlayerFromId(source)
	local society = GetSociety(society)

	TriggerEvent('urp-addonaccount:getSharedAccount', society.account, function(account)
...
RegisterServerEvent('urp-society:depositMoney')
AddEventHandler('urp-society:depositMoney', function(society, amount)
	local xPlayer = URPCore.GetPlayerFromId(source)
	local society = GetSociety(society)
...
RegisterServerEvent('urp-society:washMoney')
AddEventHandler('urp-society:washMoney', function(society, amount)
	local xPlayer = URPCore.GetPlayerFromId(source)
...
URPCore.RegisterServerCallback('urp-society:getEmployees', function(source, cb, society)
	if Config.EnableURPCoreIdentity then
...
URPCore.RegisterServerCallback('urp-society:getJob', function(source, cb, society)
...
URPCore.RegisterServerCallback('urp-society:setJob', function(source, cb, identifier, job, grade, type)

	local xPlayer = URPCore.GetPlayerFromId(source)
	local xTarget = URPCore.GetPlayerFromIdentifier(identifier)
...
URPCore.RegisterServerCallback('urp-society:setJobSalary', function(source, cb, job, grade, salary)
...
URPCore.RegisterServerCallback('urp-society:getOnlinePlayers', function(source, cb)
	local xPlayers = URPCore.GetPlayers()
	local players  = {}
...
function WashMoneyCRON(d, h, m)
	MySQL.Async.fetchAll('SELECT * FROM society_moneywash', {}, function(result)
		for i=1, #result, 1 do
			...
			local xPlayer = URPCore.GetPlayerFromIdentifier(result[i].identifier)
			if xPlayer ~= nil then
