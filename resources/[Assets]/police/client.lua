		...
		if isCop then
			...
			for i,v in ipairs(personalLockers) do
				...
				if dstCheck < 2 then
					 ...
					 if IsControlJustPressed(0, 38) then
						TriggerEvent('event:control:openpersonalpd', 1)
					 end
				end
			end
	...
	while true do 
		...
		if recentsearch == 0 then
			...
			if endDistance < 0.7 or endDistance2 < 0.7 then
				...
				currentWeapons = exports["urp-inventory"]:GetCurrentWeapons() 
...
AddEventHandler("police:rob", function()
	...
	if not exports["isPed"]:isPed("dead") then
		...
		local finished = exports["urp-taskbar"]:taskBar(3500,"Robbing",false,true)	
...
AddEventHandler( 'KneelHU', function()
	...
	if ( DoesEntityExist( player ) and not IsEntityDead( player )) then 
		...
		local finished = exports["urp-taskbar"]:taskBar(2500,"Surrendering")					
...
AddEventHandler('police:wshotgun', function()
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.0 then
			...
			local finished = exports["urp-taskbar"]:taskBar(2000,"Unracking Shotgun")
		  	if finished == 100 then
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.0 then
			...
			SetVehicleDoorOpen(targetVehicle, 0, 1, 1)
	  		loadAnimDict('anim@narcotics@trash')
			TaskPlayAnim(PlayerPedId(),'anim@narcotics@trash', 'drop_front',0.9, -8, 1900, 49, 3.0, 0, 0, 0)
			local finished = exports["urp-taskbar"]:taskBar(2000,"Unracking Rifle")
		  	if finished == 100 then
...
AddEventHandler('swatcopLoadout', function()
    ...
    if targetVehicle ~= 0 then
        ...
        if dist < 1.0 then
			...
			local finished = exports["urp-taskbar"]:taskBar(2500,"Changing to Swat")
	  		if finished == 100 then
				ChangeToSkinNoUpdatePolice("s_m_y_swat_01")		
			end
			ClearPedTasks(PlayerPedId())
...
AddEventHandler('police:gsr', function(t)
	...
	local finished = exports["urp-taskbar"]:taskBar(15000,"GSR Testing")
    if finished == 100 then
		t, distance = GetClosestPlayer()
		if(distance ~= -1 and distance < 7) then
...
AddEventHandler('police:getArrested2', function(cuffer)
	...
	local finished = 0
	if not isDead then
		finished = exports["urp-taskbarskill"]:taskBar(1200,7)
	end
...
Citizen.CreateThread(function()
    ...
    Citizen.Wait(10)
  	...
  	-- Run cuff script if police is targeting someone with a weapon and pressed E
		...
		if isMedic or isDoctor and not inmenus then
			-- up arrow
	...
	if t ~= nil and t ~= -1 then
		if(distance ~= -1 and distance < 5) then
			...
			if myjob ~= "ems" and myjob ~= "doctor" then
				local bandages = exports["urp-inventory"]:getQuantity("bandage")
				if bandages == 0 then
					return
				else
...
AddEventHandler("ems:stomachpump", function()
	...
	if t ~= nil and t ~= -1 then
		if(distance ~= -1 and distance < 5) then
			local finished = exports["urp-taskbar"]:taskBar(10000,"Inserting stomach pump 🤢", false, true)
...
local function serviceVehicle(arg, livery, isEmsWhiteListed, cb)
	...
	if arg == "help" then
		...
		elseif URPCore.GetPlayerData().job.name == "ems" then
	...
	if isCop then
		...
		if URPCore.GetPlayerData().job.name == "ems" then
	...
	Citizen.CreateThread(function()
		...
		TriggerEvent("urp-admin:runSpawnCommand",selectedSkin, livery)
	end)
end
...
AddEventHandler("police:chatCommand", function(source, args, cmd, isVehicleCmd)
	...
	if cmd == "fix" then
		local finished = exports["urp-taskbar"]:taskBar(3500,"Completing Task",false,true)	
