    ...
    exports.ghmattimysql:execute("SELECT * FROM __housing WHERE cid= ?", {cid}, function(data)
        ...
        if data[1] == nil then
            ...
            exports.ghmattimysql:execute("SELECT * FROM __housedata WHERE cid= ?", {cid}, function(chicken)
                ...
                TriggerClientEvent('hotel:createroom', src, pussy, 1, houses)
            ...
            exports.ghmattimysql:execute("SELECT * FROM __housedata WHERE cid= ?", {cid}, function(chicken)
                for k, v in pairs(chicken) do
                    if v ~= nil then
                        if v.housename ~= nil then
                            ...
                            TriggerClientEvent('hotel:createroom', src, asshole[1].roomNumber, asshole[1].roomType, houses)
                            TriggerClientEvent('hotel:SetID', src, cid)
                            return
                        end
                    end
                ...
                TriggerClientEvent('hotel:createroom', src, asshole[1].roomNumber, asshole[1].roomType, 0)
                TriggerClientEvent('hotel:SetID', src, cid)
            end)
        end
    end)
