...
function confirmSpawning(isClothesSpawn)
	...
	TriggerServerEvent("urp-clothing:get_character_current", exports['isPed']:isPed('cid'))
	TriggerServerEvent("urp-clothing:get_character_face", exports['isPed']:isPed('cid'))
    TriggerServerEvent("urp-clothing:retrieve_tats", exports['isPed']:isPed('cid'))
	...
	if isClothesSpawn then
		...
		if not exports["urp-inventory"]:hasEnoughOfItem("idcard",1,false) then
			TriggerEvent("player:receiveItem","idcard",1,true)
		end
		if not exports["urp-inventory"]:hasEnoughOfItem("mobilephone",1,false)then
...
AddEventHandler('hotel:outfit', function(id, args,sentType)
	...
	if nearClothingMotel() then
		...
		if sentType == 1 then
			...
			text = text .. ''
			TriggerEvent("urp-clothing:outfits", sentType, id, text)
		...
		elseif sentType == 3 then
			...
			TriggerServerEvent("urp-clothing:list_outfits", Player.id)
		end
	end
end)
