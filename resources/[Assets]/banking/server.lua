...
function bankBalance(player)
  return URPCore.GetPlayerFromId(player).getBank()
end
...
function deposit(player, amount)
  ...
  local user = URPCore.GetPlayerFromId(player)
...
AddEventHandler('bank:transfer', function(to, amountt)
	...
	local zPlayer = URPCore.GetPlayerFromId(to)
	local balance = 0
...
TriggerEvent('es:addCommand', 'givecash', function(source, args)
  ...
  local amount = args[2]
  local user = URPCore.GetPlayerFromId(sender)


  local playerRec = URPCore.GetPlayerFromId(reciever)
...
AddEventHandler("cash:remove", function(user, amount)
  local _source = source
  local xPlayer = URPCore.GetPlayerFromId(user)
