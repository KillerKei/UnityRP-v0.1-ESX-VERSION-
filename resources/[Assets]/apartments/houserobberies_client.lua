...
AddEventHandler("houserobberies:delete", function(id,backdoor)
...
AddEventHandler("breach", function(isFlashbang)
	...
	if isFlashbang then
		if exports["urp-inventory"]:hasEnoughOfItem("-73270376",1,false) then
...
AddEventHandler("robbery:lockpickhouse", function(isForced)
   ...
   trailer = false
	...
	Wait(1500)
	-- local finished = exports["urp-lockpicking"]:lockpick(100,5,2,10)
...
AddEventHandler('housing:findsalecid', function(checkcid, price)
    ...
    if buy then
        TriggerEvent("DoLongHudText","Attempting purchase - please wait.",1)
        if name ~= nil then
            TriggerServerEvent("housing:attemptsale",checkcid,3000,modifyingTable["house_id"],modifyingTable["house_model"])
        else
            TriggerServerEvent("housing:purchasehouse", price, checkcid, upfront, modifyingTable["house_id"],modifyingTable["house_model"]) -- house
        end
    end
...
AddEventHandler('housing:info:realtor', function(args)
	...
	if args == "unlock2" then
		TriggerServerEvent("housing:unlock",houseinfo[1])
	end

	if args == "unlock" and modifying then
		TriggerServerEvent("housing:unlockRE",houseinfo[1])
...
function PawnItems(shopnumber)
	...
	while i < 98 do
		...
		local itemcount = exports["urp-inventory"]:getQuantity(pawnTable[i])
...
--	DoScreenFadeOut(1000)
	if isFlashbang and generator then
		DoScreenFadeOut(1)
		local finished = exports["urp-taskbar"]:taskBar(2000,"Throwing Flash")
		if finished == 100 then
