...
RegisterNetEvent('urp-motels:clMotelUpdate')
AddEventHandler('urp-motels:clMotelUpdate', function(rType)
	roomType = rType
end)
...
AddEventHandler('hotel:AttemptUpgrade', function()
	if #(vector3(233.3018,-429.3954,48.07678) - GetEntityCoords(PlayerPedId())) < 3.0 then

		if myRoomType >= 2 then 
			TriggerEvent('DoLongHudText', "You already have an Integrity Apartment!", 2)
			return
		end

		if URPCore.GetPlayerData().money >= 25000 then
			...
			TriggerServerEvent('hotel:removeMoney', 25000)
		else
...
Citizen.CreateThread(function()
 	...
 	while true do
		...
		local entryUpgradesDst = #(vector3(233.3018,-429.3954,48.07678) - plyCoords)
		...
		if insideApartment or comparedst < 100 then
			...
			if entryUpgradesDst < 1.0 then
				DrawText3Ds(233.3018,-429.3954,48.07678, "~g~"..Controlkey["generalUse"][2].."~s~ Upgrade Housing (25k for tier 2.)")
