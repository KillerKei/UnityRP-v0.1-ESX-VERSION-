...
AddEventHandler('phone:addContact', function(name, number)
    ...
    MySQL.Async.execute('INSERT INTO phone_contacts (cid, name, number) VALUES (@cid, @name, @number)', {
        ['@cid'] = cid,
        ['@name'] = name,
        ['@number'] = number
    }, function(result)
...
AddEventHandler('getContacts', function(identifier, opt)
    local src = source
    local cid = getCid(src)
    MySQL.Async.fetchAll('SELECT * FROM phone_contacts WHERE cid = @cid', { ['@cid'] = cid }, function(contacts)
...
AddEventHandler('getAccountInfo', function()
    ...
    TriggerEvent('urp-license:getLicenses', source, function(licenses)
...
AddEventHandler('ReturnHouseKeys', function()
    local src = source
    local zPlayer = URPCore.GetPlayerFromId(src)
    local cid = getCid(src)
    exports.ghmattimysql:execute('SELECT * FROM user_housing WHERE `cid` = @cid', { ['@cid'] = cid}, function(result)
        TriggerClientEvent('returnPlayerKeys', src, result)
    end)
end)
