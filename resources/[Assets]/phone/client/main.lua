...
TriggerEvent("housing:sendPurchaseAttempt", tonumber(data.cid), tonumber(data.price))
end)
...
RegisterNUICallback('btnTaskGroups', function()
    local jobgrade = URPCore.PlayerData.job.grade
    local rankConverted = URPCore.PlayerData.job.grade_label

    local groupObject = {}
    table.insert(groupObject, {
        namesent = URPCore.PlayerData.job.label,
...
function findVehFromPlateAndLocate(plate)
  ...
  local gameVehicles = URPCore.Game.GetVehicles()
...
AddEventHandler("phone:Garage", function(vehs)
  ...
  local job = URPCore.GetPlayerData().job.name
...
function hasDecrypt2()
    if exports["urp-inventory"]:hasEnoughOfItem("vpnxj",1,false) and not exports["isPed"]:isPed("disabled") then
      return true
    else
      return false
    end
...
function hasDecrypt()
    if exports["urp-inventory"]:hasEnoughOfItem("decrypterenzo",1,false) or exports["urp-inventory"]:hasEnoughOfItem("decryptersess",1,false) or exports["urp-inventory"]:hasEnoughOfItem("decrypterfv2",1,false) and not exports["isPed"]:isPed("disabled") or exports["urp-inventory"]:hasEnoughOfItem(80,1,false) and not exports["isPed"]:isPed("disabled") then
      return true
    else
      return false
    end
...
function hasDevice()
    if exports["urp-inventory"]:hasEnoughOfItem("mk2usbdevice",1,false) and not exports["isPed"]:isPed("disabled") then
      return true
    else
      return false
    end
...
function hasPhone()
  ...
  if
    (
    (exports["urp-inventory"]:hasEnoughOfItem("mobilephone",1,false) or 
    exports["urp-inventory"]:hasEnoughOfItem("stoleniphone",1,false) or 
    exports["urp-inventory"]:hasEnoughOfItem("stolens8",1,false) or
    exports["urp-inventory"]:hasEnoughOfItem("stolennokia",1,false) or
    exports["urp-inventory"]:hasEnoughOfItem("stolenpixel3",1,false) or
    exports["urp-inventory"]:hasEnoughOfItem("boomerphone",1,false))
...
function hasRadio()
    if exports["urp-inventory"]:hasEnoughOfItem("radio",1,false) and not exports["isPed"]:isPed("disabled") then
      return true
    else
      return false
    end
...
function openGuiNow()
  ...
  if hasPhone() then
    ...
    SendNUIMessage({openPhone = true, hasDevice = device, hasDecrypt = decrypt, hasDecrypt2 = decrypt2, isRealEstateAgent = isREAgent, playerId = GetPlayerServerId(PlayerId())})
...
RegisterNUICallback('newPostSubmit', function(data, cb)
  if URPCore.PlayerData.job.name ~= "taxi" and URPCore.PlayerData.job.name ~= "towtruck" and URPCore.PlayerData.job.name ~= "trucker" then
    TriggerServerEvent('phone:updatePhoneJob', data.advert)
  else
    TriggerServerEvent('phone:updatePhoneJob', URPCore.PlayerData.job.name .. " | " .. data.advert)
...
AddEventHandler('getAccountInfo', function(cash, bank, licences)
    local responseObject = {
        cash = cash,
        bank = bank,
        job = URPCore.PlayerData.job.name .. ', ' .. URPCore.PlayerData.job.grade_label,
...
AddEventHandler("phone:userTwitter", function(user)
  userTwitter = user
 end)
