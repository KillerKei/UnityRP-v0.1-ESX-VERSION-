...
function runRecycle()
	...
	if not daytime then
		TriggerEvent("DoShortHudText","You can only run the factory 7am-7pm!",2)
		return
	end
	isRunningCrate = true
    ...
    FreezeEntityPosition(GetPlayerPed(-1),false)
    	...
    	TriggerEvent('player:receiveItem',"recyclablematerial", math.random(2,4))
...
listOn = false
	while true do
		...
		if GetDistanceBetweenCoords(PlayerPos, 371.45,-790.4,29.28, true) <= 3 then
			DrawText3Ds(371.45,-790.4,29.28, "...")
			if IsControlJustPressed(0, 38) then
			TriggerEvent("server-inventory-open", "31", "Craft"); 
			end
		end

		if GetDistanceBetweenCoords(PlayerPos, 1689.34, 2552.22, 45.56, true) <= 3 then
			DrawText3Ds(1689.34, 2552.22, 45.56, "...")
			if IsControlJustPressed(0, 38) then
			TriggerEvent("server-inventory-open", "25", "Craft"); 
			end
		end

		if GetDistanceBetweenCoords(PlayerPos, 1109.7, -2007.71, 31.04, true) <= 3 then
			DrawText3Ds(1109.7, -2007.71, 31.04, "...")
			if IsControlJustPressed(0, 38) then
		...
		--	end
			...
			if GetDistanceBetweenCoords(PlayerPos, 2221.7973632813,5577.0966796875,53.844783782959, true) <= 10 then		
				...
				if JustDriedWeed then
					TriggerEvent("DoShortHudText","You are too tired from drying weed, either wait 15 minutes or get help!")
				else
			...
			if GetDistanceBetweenCoords(PlayerPos, 3803.58, 4444.46, 4.1, true) <= 15 then						
				...
				if JustPickedWeed then
					TriggerEvent("DoShortHudText","You are too tired from gathering weed, either wait 15 minutes or get help!")
				else
			...
			if GetDistanceBetweenCoords(PlayerPos, 5.87, -1601.48, 29.3, true) <= 5 then						
				...
				local smallbud = exports["urp-inventory"]:hasEnoughOfItem("freshmeat",5,false)
				if smallbud then
	...
	if not leftFishing then
		...
		if fish then 
			...
			if itemRandom < 550 then
				...
				TriggerEvent("DoShortHudText","You just caught a "..fishTable[fishNumber].name.." that was " .. (fishsize / 10) .. "cm long.",1)
				TriggerEvent("player:receiveItem","freshmeat",math.random(3))
			end

		else
			TriggerEvent("DoShortHudText","You took too long.",2)
		end
	end
