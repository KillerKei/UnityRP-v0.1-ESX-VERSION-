...
Citizen.CreateThread(function()
    ...
    exports.ghmattimysql:execute("SELECT * FROM `banks`", function(bankssql)
    ...
    exports.ghmattimysql:execute("SELECT * FROM `bank_accounts` WHERE `account_type` = 'Buisness'", function(accts)
        buis = #accts
        if accts[1] ~= nil then
...
URPCore.RegisterServerCallback('rl-banking:server:requestBanks', function(source, cb)
    repeat Wait(0) until banks ~= nil
    cb(banks)
end)

URPCore.RegisterServerCallback('rl-banking:server:checkMoneyBagCount', function(source, cb)
    local src = source
...
function checkAccountExists(acct, sc)
    ...
    exports.ghmattimysql:execute("SELECT * FROM `bank_accounts` WHERE `account_number` = '" .. acct .. "' AND `sort_code` = '" .. sc .. "'", function(exists)
...
AddEventHandler('rl-banking:server:unpackMoneyBag', function(item)
    local _src = source
    if item ~= nil then
        local xPlayer = URPCore.GetPlayerFromId(_src)
        local xPlayerCID = exports["isPed"]:isPed("cid")
...
function getCharacterName(identifier)
    local name
    exports.ghmattimysql:execute("SELECT * FROM users WHERE identifier = @identifier", {['@identifier'] = identifier}, function(exists)
        if exists[1] ~= nil then 
...
AddEventHandler('rl-banking:initiateTransfer', function(data)
    --[[
    local _src = source
    local _startChar = URPCore.GetPlayerFromId(_src)
...
URPCore.RegisterServerCallback('rl-banking:getBankingInformation', function(source, cb)
    local src = source
    local xPlayer = URPCore.GetPlayerFromId(src)
        ...
        if (xPlayer) then
            local banking = {
                   -- ['name'] = char.firstname .. " " .. char.lastname,
                    ...
                    ['accountinfo'] = URPCore.GetPlayerFromId(),
                }
...
AddEventHandler('rl-banking:doQuickWithdraw', function(amount, branch)
    ...
    if tonumber(amount) <= currentCash then
        local cash = xPlayer.addMoney(tonumber(amount))
        local bank = xPlayer.removeAccountMoney('bank', tonumber(amount))
        if cash then 
            TriggerClientEvent('rl-banking:openBankScreen', _src)
            TriggerClientEvent('DoLongHudText', _src, 'You made a cash withdrawal of $'..amount..' successfully.')
...
AddEventHandler('rl-banking:savingsWithdraw', function(amount)
    ...
    local xPlayer = URPCore.GetPlayerFromId(src)
    while xPlayer == nil do Wait(0) end
    local currentSavings = savingsAccounts[URPCore.GetPlayerFromId()].GetBalance()
    
    if tonumber(amount) <= currentSavings then
        local savings = savingsAccounts[URPCore.GetPlayerFromId()].RemoveMoney(tonumber(amount), 'Savings to Current Account Transfer')
        local bank = xPlayer.addAccountMoney('bank', tonumber(amount), 'banking-quick-withdraw')
        while bank == nil do Wait(0) end
        while savings == nil do Wait(0) end
        TriggerClientEvent('rl-banking:openBankScreen', src)
...
AddEventHandler('rl-banking:createSavingsAccount', function()
    local src = source
    local xPlayer = URPCore.GetPlayerFromId(src)
    local success = createSavingsAccount(URPCore.GetPlayerFromId())
