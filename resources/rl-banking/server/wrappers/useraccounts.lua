...
function generateCurrent(cid)
    ...
    exports.ghmattimysql:execute("SELECT * FROM `bank_cards` WHERE `citizenid` = '" .. self.cid .."'", function(getCurrentAccount)
    ...
    exports.ghmattimysql:execute("SELECT * FROM `bank_statements` WHERE `account` = 'Current' AND `citizenid` = '" .. self.cid .. "' ORDER BY `record_id` DESC LIMIT 30", function(bankStatement)
    ...
    self.updateItemPin = function(pin)
        local processed = false
        local success
        exports.ghmattimysql:execute("SELECT * FROM `stored_items` WHERE `metaprivate` LIKE '%\"cardnumber\":"..self.cardNumber.."%' AND `metaprivate` LIKE '%\"account\":"..self.account.."%' AND `metaprivate` LIKE '%\"sortcode\":"..self.sortcode.."%' AND `type` = 'Bankcard' LIMIT 1", function(item)
            if item[1] ~= nil then
                itemFound = true
    ...
    rTable.generateNewCard = function(pin, scc)
        ...
        if not self.cardActive then
            ...
            exports.ghmattimysql:execute("UPDATE `bank_cards` SET `cardNumber` = '" .. cardNumber .. "', `cardPin` = '" .. pinSet .. "', `cardDecrypted` = 'false', `cardActive` = 1, `cardLocked` = 0, `cardType` = '" .. friendlyName .. "' WHERE `citizenid` = '" ..self.cid .. "' AND `record_id` = '" .. self.aid .. "'", function(rowsChanged)
                ...
                if self.source ~= -1 then
                    ...
                    local xPlayer = URPCore.GetPlayerFromId(self.source)
...
function generateSavings(cid)
    ...
    exports.ghmattimysql:execute("SELECT * FROM `bank_accounts` WHERE `citizenid` = '@cid' AND `account_type` = 'Savings'", function(getSavingsAccount)
        if getSavingsAccount[1] ~= nil then
            self.aid = getSavingsAccount[1].record_id
            self.balance = getSavingsAccount[1].amount
        end
        
        exports.ghmattimysql:execute("SELECT * FROM `bank_statements` WHERE `account` = 'Savings' AND `citizenid` = '@cid' ORDER BY `record_id` DESC LIMIT 30", function(stats)
    ...
    self.saveAccount = function()
        exports.ghmattimysql:execute("UPDATE `bank_accounts` SET `amount` = '" .. self.balance .. "' WHERE `citizenid` = '@cid' AND `record_id` = '" .. self.aid .. "'")
        if success == 1 then
            return true
        else
            return false
    ...
    rTable.AddMoney = function(amt, text)
        if type(amt) == "number" and text then
            ...
            local time = os.date("%Y-%m-%d %H:%M:%S")
            MySQL.Async.execute("INSERT INTO `bank_statements` (`citizenid`,  `account`, `deposited`, `withdraw`, `balance`, `date`, `type`) VALUES ('" .. self.cid .. "', 'Saving','" .. amt .. "', '', '" .. self.balance .. "', '" .. time .. "', '" .. text .. "')")
    ...
    rTable.RemoveMoney = function(amt, text)
        if type(amt) == "number" and text then
            if amt <= self.balance then
                ...
                local time = os.date("%Y-%m-%d %H:%M:%S")
                exports.ghmattimysql:execute("INSERT INTO `bank_statements` (`citizenid`,  `account`  `deposited`, `withdraw`, `balance`, `date`, `type`) VALUES ('" .. self.cid .. "', 'Saving', '','" .. amt .. "', '" .. self.balance .. "', '" .. time .. "', '" .. text .. "')")
...
function createSavingsAccount(cid)
    ...
    local success = false
    exports.ghmattimysql:execute("INSERT INTO `bank_accounts` (`citizenid`, `amount`, `account_type`) VALUES (@cid, '0', 'Savings')", {
        ['@cid'] = identifier,
    }, function(inserted)
        savingsAccounts[identifier] = generateSavings(identifier)
        success = true
        completed = true
    end)
