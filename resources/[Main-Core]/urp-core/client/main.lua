...
Citizen.CreateThread(function()
	while true do
		...
		if paycheckReady == true and stackTwice == false and stackThree == false then
			URPCore.TriggerServerCallback('paycheck:checkSalary', function(result)
...
AddEventHandler('urp:playerLoaded', function(xPlayer)
	URPCore.PlayerLoaded = true
	URPCore.PlayerData = xPlayer

	if Config.EnableHud then
		TriggerEvent('es:setMoneyDisplay', 0.0)
		URPCore.UI.HUD.SetDisplay(0.0)
		for k,v in ipairs(xPlayer.accounts) do
			local accountTpl = '<div><img src="img/accounts/' .. v.name .. '.png"/>&nbsp;{{money}}</div>'

			URPCore.UI.HUD.RegisterElement('account_' .. v.name, k - 1, 0, accountTpl, {
				money = 0
			})

			URPCore.UI.HUD.UpdateElement('account_' .. v.name, {
				money = URPCore.Math.GroupDigits(v.money)
			})
		end
		...
		URPCore.UI.HUD.RegisterElement('job', #xPlayer.accounts, 0, jobTpl, {
			job_label   = '',
			grade_label = ''
		})

		URPCore.UI.HUD.UpdateElement('job', {
...
AddEventHandler('playerSpawned', function()
	...
	-- Restore position
	if URPCore.PlayerData.lastPosition then
		SetEntityCoords(playerPed, URPCore.PlayerData.lastPosition.x, URPCore.PlayerData.lastPosition.y, URPCore.PlayerData.lastPosition.z)
	end

	TriggerEvent('urp:restoreLoadout') -- restore loadout
...
AddEventHandler('urp:onPlayerDeath', function()
	isDead = true
end)
...
RegisterNetEvent('urp:setAccountMoney')
AddEventHandler('urp:setAccountMoney', function(account)
	for k,v in ipairs(URPCore.PlayerData.accounts) do
		if v.name == account.name then
			URPCore.PlayerData.accounts[k] = account
			break
		end
	end

	if Config.EnableHud then
		URPCore.UI.HUD.UpdateElement('account_' .. account.name, {
			money = URPCore.Math.GroupDigits(account.money)
		})
	end
end)
...
AddEventHandler('es:activateMoney', function(money)
	URPCore.PlayerData.money = money
end)

RegisterNetEvent('urp:setJob')
AddEventHandler('urp:setJob', function(job)
	URPCore.PlayerData.job = job
end)

-- Commands
RegisterNetEvent('urp:teleport')
AddEventHandler('urp:teleport', function(pos)
	pos.x = pos.x + 0.0
	pos.y = pos.y + 0.0
	pos.z = pos.z + 0.0
...
RegisterNetEvent('urp:loadIPL')
AddEventHandler('urp:loadIPL', function(name)
	Citizen.CreateThread(function()
		LoadMpDlcMaps()
		RequestIpl(name)
	end)
...
RegisterNetEvent('urp:unloadIPL')
AddEventHandler('urp:unloadIPL', function(name)
	Citizen.CreateThread(function()
		RemoveIpl(name)
	end)
end)

RegisterNetEvent('urp:playAnim')
AddEventHandler('urp:playAnim', function(dict, anim)
	Citizen.CreateThread(function()
		local playerPed = PlayerPedId()
...
RegisterNetEvent('urp:playEmote')
AddEventHandler('urp:playEmote', function(emote)
	Citizen.CreateThread(function()
...
RegisterNetEvent('urp:spawnVehicle')
AddEventHandler('urp:spawnVehicle', function(model)
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)

	URPCore.Game.SpawnVehicle(model, coords, 90.0, function(vehicle)
...
RegisterNetEvent('urp:spawnObject')
AddEventHandler('urp:spawnObject', function(model)
	local playerPed = PlayerPedId()
	local coords    = GetEntityCoords(playerPed)
	local forward   = GetEntityForwardVector(playerPed)
	local x, y, z   = table.unpack(coords + forward * 1.0)

	URPCore.Game.SpawnObject(model, {
		x = x,
		y = y,
		z = z
	}, function(obj)
...
RegisterNetEvent('urp:pickup')
AddEventHandler('urp:pickup', function(id, label, player)
	...
	--[[URPCore.Game.SpawnLocalObject('prop_money_bag_01', {
		x = x,
		y = y,
		z = z - 2.0,
	}, function(obj)
...
RegisterNetEvent('urp:removePickup')
AddEventHandler('urp:removePickup', function(id)
	URPCore.Game.DeleteObject(pickups[id].obj)
	pickups[id] = nil
end)

RegisterNetEvent('urp:spawnPed')
AddEventHandler('urp:spawnPed', function(model)
...
RegisterNetEvent('urp:deleteVehicle')
AddEventHandler('urp:deleteVehicle', function()
	local playerPed = PlayerPedId()
	local vehicle   = URPCore.Game.GetVehicleInDirection()
	...
	while true do
		...
		if URPCore.PlayerLoaded and isPlayerSpawned then
			...
			if not IsEntityDead(playerPed) then
				URPCore.PlayerData.lastPosition = {x = coords.x, y = coords.y, z = coords.z}
			end
		end
