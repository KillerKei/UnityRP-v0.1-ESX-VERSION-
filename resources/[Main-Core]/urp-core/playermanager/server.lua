...
MySQL.ready(function()
	...
	for i=1, #result2 do
		if URPCore.Jobs[result2[i].job_name] then
			...
			print(('urp-core: invalid job "%s" from table job_grades ignored!'):format(result2[i].job_name))
		end
	end
	...
	for k,v in pairs(URPCore.Jobs) do
		if next(v.grades) == nil then
			...
			print(('urp-core: ignoring job "%s" due to missing job grades!'):format(v.name))
		end
	end
end)
